//Prime number locator(method 1) training by Andrew Alston
...
func FindPrime(Min, Max int) []int {
	...
	for i := Min; i < Max; i++{
		...
		IsPrime = true
		//Since anything is divided by a number greater than half of itself is always going to produce
		...
		//If this isn't prime number, append it to our result list
		if IsPrime {
			res = append(res, i)
		}
	}
	...
	func main(){
		...
		//Note flags are always pointers, so we have to dereference them, hence the asterix
		...
		if *TimeExecution {
			fmt.Printf("Took us %s to find all the primes in a range of %d numbers\n", time.Since(StartTime), *Maximum-*Minimum)
		}
