// Binary level routines as demonstrated by Andrew Alston
//David Gatheru
...
//to make printing and reading easier
...
// ExclusiveOr performs a XOR (Exclusive Or) between two numbers and prints the ressults.
...
// When a bitwise OR is performed, the result will be 1 if either of the bits were 1, otherwise the result will be 0

func BitwiseOr(num1, num2 uint16) uint16 {
	fmt.Printf("Perorming Lofical OR between %d and %d\n", num1, num2)
	fmt.Printf("Binary [%2d]: %16b\n", num1, num1)
	fmt.Printf("Binary [%2d]: %16b\n", num2, num2)
	fmt.Printf("Result [%2d]: %016b\n", num1|num2, num1|num2)
	return num1 | num2
}
...
//and dropping the left most bits. This effecively doubles the num in question
...
func RightShift(num1, num2 uint16) uint16{
	...
	fmt.Printf("Binary [%2d]: %016b\n", num1, num1)
	fmt.Printf("Results [%2d]: %016b\n", num1>>num2, num1>>num2)
	return num1 >> num2
}

// TestBit sets to see if binary bit is set in the given  number. It assumes
...
func TestBit(num1 interface{}, bit uint8) (bool, error) {
	switch num1.(type) {
	case uint8:
		fmt.Printf("Binary [%3d] [%08b] >> %d [bit %d] == [%3d] %08b [%v]\n", num1, num1, bit, 7-bit, num1.(uint8)>>(7-bit),(num1.(uint8)>>(7-bit))&1 == 1)
		return (num1.(uint8)>>(7-bit))&1 == 1, nil
	case uint16:
		fmt.Printf("Binary [%3d] [%08b] >> %d [bit %d] == [%3d] %08b [%v]\n", num1, num1, bit, 15-bit, num1.(uint16)>>(15-bit),(num1.(uint16)>>(15-bit))&1 == 1)
		return (num1.(uint16)>>uint16(15-bit))&1 == 1, nil
	case uint32:
		fmt.Printf("Binary [%3d] [%08b] >> %d [bit %d] == [%3d] %08b [%v]\n", num1, num1, bit, 31-bit, num1.(uint32)>>(31-bit), (num1.(uint32)>>(31-bit))&1 == 1)
		return (num1.(uint32)>>uint32(31-bit))&1 == 1, nil
	case uint64:
		fmt.Printf("Binary [%3d] [%08b] >> %d [bit %d] == [%3d] %08b [%v]\n", num1, num1, bit, 63-bit, num1.(uint64)>>(63-bit), (num1.(uint64)>>(63-bit))&1 == 1)
...
// CombineContains tests if a number is logically part of another number at a binary level. As an example:
...
// acs as an incrementer for a shift in the constant declarations
func TestByStaticMask(num1, bit uint8) bool {
	// We set bit7 constant as a zero and then set the subsequent constants
